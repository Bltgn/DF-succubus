...
eventful.onReactionComplete.summonHfs = function(reaction, unit, input_items, input_reagents, output_items, call_native)
	local selection
	local key = 1
	local demonId = {}

	if not reaction.code:find('SUMMON_HFS') then return nil end
	...
	dfhack.run_script('succubus/summoning', unit.id, demonId[selection], 'true')
end